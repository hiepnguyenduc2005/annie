...
| [YOLO-World](https://github.com/AILab-CVC/YOLO-World) via Ultralytics, with the [CLIP](https://github.com/ultralytics/CLIP) text encoder | Open-vocabulary detection from text prompts (doors, walls, windows, stairs, household objects). | Optional mode of `robot/dog/perception/objects.py`. Weights `yolov8s-worldv2.pt` from the `ultralytics/assets` release [`v8.4.0`](https://github.com/ultralytics/assets/releases/download/v8.4.0/yolov8s-worldv2.pt), sha256 `9b2c17ab6124a913e9b3a5c170617920d91b0f01111a8479da69f00e2cf27792`, kept under the ignored `.cache/yolo/`; Ultralytics code and weights AGPL-3.0. CLIP fork pinned at commit `a13192f8cb767260d7dfd98c843b0716593169e7` (MIT); it fetches OpenAI CLIP ViT-B/32 (MIT) on first use and is needed only to embed the vocabulary once. Thresholds and timings are in the module docstring; door thresholds were measured on public photos, not yet on the dog's camera. |
