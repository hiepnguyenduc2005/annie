...
- `ANNIE_API_TOKEN`: optional bearer token. Without it, data routes accept only
  loopback and private LAN clients. With it, every data request requires `Authorization: Bearer …`.
...
disabled. Request bodies are limited to 600,000 bytes. Localhost and private LAN IP destinations are accepted automatically.
No host-list setting or token is needed for private LAN clients.
...
`127.0.0.1`). Private LAN IP destinations work automatically for both the
iPhone and robot callbacks; internal callbacks still require their secret.
