...
class Bridge:
    def __init__(self, viewer, app, brain, *, perception="disabled", max_inferences=20,
                 ...
                 continuous_local=False, memory=None):
    ...
    async def persist_hosted(self, perception):
        from robot.app_backend.app.models import Perception
        try:
            await self.memory.upsert(Perception.model_validate(perception))
        except Exception as exc:
    ...
    def warn(self, area, exc):
        ...
        if area in ('agent','vision') and isinstance(exc,httpx.HTTPStatusError):
            try:
                ...
                # These messages are generated by our own brain boundary;
                # never relay an upstream response body or arbitrary text.
                allowed=('Planner provider returned an invalid response',
                    'Planner provider returned an invalid plan shape',
                    'Planner provider did not complete a plan',
                    'Planner proposed an unknown waypoint',
                    'Planner provider request failed (HTTP 429)',
                    'Planner provider timed out')
                if detail in allowed: self.last_error += ': '+detail
        ...
        try:
            ...
            if self.ingest_accepted:
                ...
                if self.memory and (self.memory_task is None or self.memory_task.done()):
                    self.memory_task = asyncio.create_task(self.persist_hosted(perception))
            outstanding=await self.request(self.app,'GET','/commands')
            status=await self.request(self.app,'GET','/status')
            ...
            if command:
                ...
                # operator review; never retry a potentially accepted command.
                self.pending_agent_command = 'delivery-uncertain'
                if command['cmd']=='say':
            ...
            self.history=(self.history+[{'ts':frame['ts'],'frame_id':frame['frame_id'],
                'action':action,'execution':detail,'command_id':receipt['command_id'] if receipt else None,
    ...
    async def tick(self, *, wait_vision=False):
        ...
        if (state.get("ready") and self.perception in ("vision",'agent') and
            ...
            # Once a risky camera observation exists, the incident policy
            ...
            if confirming and self.agent_state:
                self.agent_state['execution']='Confirming posture with another fresh camera image'
            self.vision_task = asyncio.create_task(self.think(state) if self.perception=='agent' and not confirming else self.infer())
...
async def run(args):
    ...
    async with httpx.AsyncClient(base_url=args.viewer_url, timeout=5, trust_env=False) as viewer, \
        ...
        if args.continuous_local:
            health = await brain.get('/health')
            health.raise_for_status()
            if health.json().get('mode') != 'local':
                raise ValueError('Continuous operation is permitted only with a local brain')
        from robot.simulation.hosted_memory import from_env
        bridge = Bridge(viewer, app, brain, perception=args.perception,
                        ...
                        status_file=args.status_file, continuous_local=args.continuous_local, memory=from_env())
...
def main():
    ...
    parser.add_argument('--continuous-local', action='store_true', help='Uncapped local planning; refuses a cloud brain')
