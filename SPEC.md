...
| REQ-012 | SwiftUI app is family-only: Jeanine is not a user of it, and every screen (Reminders, Ask Annie, Activity, Profile) is shown to every signed-in family member. | Sign-in picks which family member owns this phone from the fixed household `app_backend` recognizes (`zach`, `ellis`); a second sign-in on the same device is rejected; the choice is stored on the device only and used as the message `author_id` (DEC-011, superseding the app-user/dog-user split and the resident/family audience picker in DEC-008/DEC-009). Server-side storage, cross-device pairing, and adding family members beyond the fixed household remain open. |
| REQ-015 | Ask Annie is one screen for both an instant lookup and an in-person check, and the two are never conflated. | Asking anything (`POST /api/ask`) answers immediately from recorded observations and never dispatches the robot; when nothing matches, the fallback answer is shown and a distinct, explicit "have Annie check with Jeanine in person" action is offered, which only then dispatches a run (REQ-010) with the same text (DEC-012). |
